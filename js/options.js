...
const getDocLink = wrapper({
    type: "app.get",
    what: "doclink"
}, "link");
const getInfo = wrapper({
    type: "app.get"
}, "what");
const getPref = wrapper({
    type: "prefs.get"
}, "key");
const togglePref = wrapper({
    type: "prefs.toggle"
}, "key");
const getSubscriptions = wrapper({
        type: "subscriptions.get"
    },
    "downloadable", "special");
const removeSubscription = wrapper({
...
const addSubscription = wrapper({
    ...
    "url", "title", "homepage");
const toggleSubscription = wrapper({
        type: "subscriptions.toggle"
    },
    "url", "keepInstalled");
const updateSubscription = wrapper({
    type: "subscriptions.update"
}, "url");
const importRawFilters = wrapper({
...
const getFilters = wrapper({
    type: "filters.get"
}, "subscriptionUrl");
const removeFilter = wrapper({
    type: "filters.remove"
}, "text");
...
function loadOptions() {
    ...
    // Add event listeners
    $("#updateFilterLists")
        .click(updateFilterLists);
    $("#startSubscriptionSelection")
        .click(startSubscriptionSelection);
    $("#subscriptionSelector")
        .change(updateSubscriptionSelection);
    $("#addSubscription")
        .click(addSubscriptionClicked);
    $("#acceptableAds")
        .click(toggleAcceptableAds);
    $("#whitelistForm")
        .submit(addWhitelistDomain);
    $("#removeWhitelist")
        .click(removeSelectedExcludedDomain);
    $("#customFilterForm")
        .submit(addTypedFilter);
    $("#removeCustomFilter")
        .click(removeSelectedFilters);
    $("#rawFiltersButton")
        .click(toggleFiltersInRawFormat);
    $("#importRawFilters")
        .click(importRawFiltersText);
    ...
    getInfo("features", features => {
        if (!features.devToolsPanel)
            document.getElementById("showDevtoolsPanelContainer")
            .hidden = true;
    });
    getPref("notifications_showui", notifications_showui => {
        if (!notifications_showui)
            document.getElementById("shouldShowNotificationsContainer")
            .hidden = true;
    });
...
function reloadFilters() {
    ...
    getSubscriptions(true, false, subscriptions => {
        for (let subscription of subscriptions) {
            if (subscription.url == acceptableAdsUrl)
                $("#acceptableAds")
                .prop("checked", !subscription.disabled);
            else
    ...
    getSubscriptions(false, true, subscriptions => {
        document.getElementById("userFiltersBox")
            .innerHTML = "";
        document.getElementById("excludedDomainsBox")
            .innerHTML = "";
        for (let subscription of subscriptions)
...
function startSubscriptionSelection(title, url) {
    ...
    if (typeof url != "undefined") {
        ...
        document.getElementById("customSubscriptionTitle")
            .value = title;
        document.getElementById("customSubscriptionLocation")
            .value = url;
    }
    updateSubscriptionSelection();
    document.getElementById("addSubscriptionContainer")
        .scrollIntoView(true);
}
...
function addSubscriptionClicked() {
    ...
    if (data)
        ...
        if (!/^https?:/i.test(url)) {
            ...
            $("#customSubscriptionLocation")
                .focus();
            return;
        }
        let title = document.getElementById("customSubscriptionTitle")
            .value.trim();
        if (!title)
            title = url;
...
function findSubscriptionElement(subscription) {
    for (let child of document.getElementById("filterLists")
            .childNodes)
...
function addWhitelistDomain(event) {
    ...
    let domain = document.getElementById("newWhitelistDomain")
        .value.replace(/\s/g, "");
    document.getElementById("newWhitelistDomain")
        .value = "";
    if (!domain)
        return;
...
function removeSelectedExcludedDomain(event) {
    event.preventDefault();
    let remove = [];
    for (let option of document.getElementById("excludedDomainsBox")
            .options)
        if (option.selected)
...
function toggleFiltersInRawFormat(event) {
    ...
    if (rawFilters.style.display != "table-row") {
        ...
        for (let option of document.getElementById("userFiltersBox")
                .options)
            filters.push(option.value);
    } else {
        rawFilters.style.display = "none";
    }
    document.getElementById("rawFiltersText")
        .value = filters.join("\n");
}
// Imports filters in the raw text box
function importRawFiltersText() {
    let text = document.getElementById("rawFiltersText")
        .value;
...
function addSubscriptionEntry(subscription) {
    ...
    document.getElementById("filterLists")
        .appendChild(element);
}
